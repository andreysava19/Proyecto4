...
def modulador(bits, fc, mpp):
    ...
    # Se definen el vector bit a recorrer
    bit1 = bits.reshape(-1, 4)
    ...
    for b in bit1:
        ...
        # Nota: Moduladora = 1 para portadoras positivas
        # y Moduladora = 0 para portadoras negativas
        
        # Para los bits b1 y b2
        #00
        if (b[0] == 0) and (b[1] == 0):
        ...
        # Para los bits b3 y b4
        #00
        if (b[2] == 0) and (b[3] == 0):
            senal_Tx[(i+2)*mpp : (i+3)*mpp] = portadora2*3
            moduladora[(i+2)*mpp : (i+3)*mpp] = 3
        #01  
        if (b[2] == 0) and (b[3] == 1):
            senal_Tx[(i+2)*mpp : (i+3)*mpp] = portadora2*1
        ...
        # Como ya utilizaron los primeros 4 bits se le suma 4 al contador para
        # poder seguir con los siguientes 4 bits
        i = i + 4

 # 5. Calcular la potencia promedio de la señal modulada
    Pm = (1 / (N*Tc)) * np.trapz(pow(senal_Tx, 2), t_simulacion)
    
    # Como son dos portadoras y como viajan juntas, se procede a sumar 
    # las dos portadoras constuídas
...
def demodulador(senal_Rx, portadora, mpp):
    ...
    # Creación de matriz para la señal de entrada
    matriz_entrada = np.zeros((int(N/4),4))
    ...
    # Demodulación
    # i sirve para recorrer las filas de la matriz de entrada
    # n sirve para recorrer el vector senal_Rx
    n = 0
    
    # rango es el rango que debe recorrer el el for
    rango = int(N/4)
    
    for i in range(rango):
        # Producto interno de dos funciones
        # Para la portadora 1
        producto1 = senal_Rx[n*mpp : (n+1)*mpp] * portadora
        Ep1 = np.sum(producto1)/10
        senal_demodulada1[n*mpp : (n+1)*mpp] = Ep1
        
        # Para la portadora 2
        producto2 = senal_Rx[(n+2)*mpp : (n+3)*mpp] * portadora
        Ep2 = np.sum(producto2)/10
        senal_demodulada2[(n+2)*mpp : (n+3)*mpp] = Ep2
        ...
        # Amplitud = 3
        if (Ep1 > 2):
            matriz_entrada[i,0] = 1
            matriz_entrada[i,1] = 0
        
        # Amplitud = -3
        if (Ep1 < -2):
            matriz_entrada[i,0] = 0
            matriz_entrada[i,1] = 0
        
        # Amplitud = 1
        if (Ep1 < 2) and (Ep1 > 0):
            matriz_entrada[i,0] = 1
            matriz_entrada[i,1] = 1
        ...
        # Amplitud = 3
        if (Ep2 > 2):
            matriz_entrada[i,2] = 0
            matriz_entrada[i,3] = 0
        
        # Amplitud = -3
        if (Ep2 < -2):
            matriz_entrada[i,2] = 1
            matriz_entrada[i,3] = 0
    ...
    # Se convierte la matriz de entrada en un vector
    bits_entrada = matriz_entrada.reshape(-1)
    
    # Como se tienen dos señales demoduladas se suman
    senal_demoduladaT = senal_demodulada1 + senal_demodulada2
    
    return bits_entrada.astype(int), senal_demoduladaT


# In[129]:


# Simulación del sistema de comunicaciones con modulación 16-QAM
import numpy as np
import matplotlib.pyplot as plt
import time
...
X_t = np.empty((16, len(t_muestra)))	   # 16 funciones del tiempo x(t) 
...
print("Conclusiones:")
print("Como se observa en la gráfica Realizaciones del proceso aleatorio, el promedio de realizaciones (línea negra) y el valor teórico (línea turquesa) presentan el mismo valor; por lo tanto, en la señal modulada senal_Tx hay ergodicidad.")
...
# 4.3 Densidad espectral de potencia
#Realización del proceso = señal_Tx (se quita el E(valor deseado) y se deja unicamente el argumento((absXtw)^2)
